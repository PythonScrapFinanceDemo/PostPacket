该程序主要使用了BeautifulSoup,requests,pandas,datetime,calendar这几个python库。
requests主要负责模拟浏览器发送post报文.
BeautifulSoup负责得到html代码,解析出需要的数据字段
...
总体思路:
         (1)将每个功能模块封装好,方便其他模块调用.
         
         (2)采用循环调用的思想:
                      
                       *1*每个线程对一个组的一个交易日期的一个页面进行爬取,存储数据.
                      
                       *2*根据获得一个组的一个交易日期共有n个网页页面时,调用*1*,循环n次开启n个线程同时爬取存储数据.
                      
                       *3*根据获得一个组的m个交易日期时,调用*2*,循环m次开启m个交易日的爬取存储.
                      
                       *4*根据排行榜前10个组,调用*3*,循环10次开启10个组的爬取存储.
...
1----数据合并：
  ...
  说明:仅一个deal_csv_OK.py文件
        ...
        all_to_one(nameList):
                                ...
                                结果:生成namelist中所有csv文件合并后的总的csv文件
...
2----生成客户代码:
        ...
        get_id():
                                ...
                                功能:根据make_unique()返回的列表,对客户进行编码
